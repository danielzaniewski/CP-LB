WITH
    MATERIAL_LIST AS (
        ...
        FROM (
                 SELECT
                     ROW_NUMBER() OVER (PARTITION BY MATERIAL_NUMBER ORDER BY UPDATED.TIME) AS rn,
...
-- SQ1 (Subquery-1) for Return Quantity
    SQ1 AS (
        SELECT
            'DataOne' AS SOURCE_SYSTEM_CODE,    -- DataOne-PBC - check whether specific source is required
            ...
            'SAP' AS OPERATIONAL_ACCOUNT_IDENTIFIER_1_SOURCE_SYSTEM_CODE, -- SAP-PBC-C - check whether specific source is required
...
SELECT
    ...
    CAST('0.00' AS NUMERIC) AS TOTAL_ORDER_QUANTITY_LOCAL
FROM
    bcs-customer360-prod.staging_eu.SDD_PI_UNIQUE
WHERE
    1=1
  AND COMP_CODE IN ('0085',
    '2926')
...
SELECT
    ...
    CAST('0.00' AS NUMERIC)AS TOTAL_ORDER_QUANTITY_LOCAL
FROM
    bcs-customer360-prod.staging_eu.SDB_PI_UNIQUE
WHERE
    1=1
  AND COMP_CODE IN ('0085',
    '2926')
...
SELECT
   ...
   cast(SUM(CASE GASSP.LOCAL_CURRENCY_CODE WHEN'EUR' THEN GASSP.GROSS_SALES_AMOUNT_LOCAL
    ...
    GASSP.LOCAL_CURRENCY_CODE AS GROSS_SALES_AMOUNT_LOCAL_CURRENCY_CODE, --local currency to be checked if ccy within scope
   cast(SUM(CASE GASSP.LOCAL_CURRENCY_CODE WHEN'EUR' THEN GASSP.GROSS_SALES_AMOUNT_LOCAL
    ELSE
    (GASSP.GROSS_SALES_AMOUNT_LOCAL * CCR.exchange_rate)
    END
    ...
    'EUR' AS NET_SALES_AMOUNT_LOCAL_CURRENCY_CODE,
    cast(SUM(CASE GASSP.LOCAL_CURRENCY_CODE WHEN'EUR' THEN GASSP.NET_SALES_AMOUNT_LOCAL
    ELSE
    (GASSP.NET_SALES_AMOUNT_LOCAL * CCR.exchange_rate)
    END
    ) as numeric) AS NET_SALES_AMOUNT_GLOBAL,
    GASSP.LOCAL_CURRENCY_CODE AS NET_SALES_AMOUNT_GLOBAL_CURRENCY_CODE,
...
,OPL.OPL_ACCOUNT_ID
,CRM.opl_source_system_code  --changed to CRM as source
...
FROM
    OPL OPL
        JOIN
    `bcs-customer360-nonprod.test_dataset.crm_opl_xref` CRM
    ON
