WITH
    MATERIAL_LIST AS (
        ...
        FROM (
                 ...
                 FROM
                     bcs-customer360-nonprod.csw_always_prod.materials_MaterialsV2
                 WHERE
                     SOURCE = 'BC'
    ...
    CCR AS (
        SELECT
            DISTINCT from_currency_code,
                     ...
                     exchange_rate
        FROM
            `bcs-customer360-nonprod.csw.core_exchange_rate_cal`
        WHERE
...
-- SQ1 (Subquery-1) for Return Quantity
    SQ1 AS (
        SELECT
            ...
            'SAP-QBC-C' AS OPERATIONAL_ACCOUNT_IDENTIFIER_1_SOURCE_SYSTEM_CODE,
        ...
        FROM
            bcs-customer360-nonprod.h2r_stage.SDO_PIS_UNIQUE
WHERE
    1=1
...
SELECT
    ...
    CAST('0.00' AS NUMERIC) AS TOTAL_ORDER_QUANTITY_LOCAL
FROM
    bcs-customer360-nonprod.h2r_stage.SDD_PI_UNIQUE
WHERE
    1=1
  AND COMP_CODE IN ('0085',
    '2926')
...
SELECT
    ...
    CAST('0.00' AS NUMERIC)AS TOTAL_ORDER_QUANTITY_LOCAL
FROM
    bcs-customer360-nonprod.h2r_stage.SDB_PI_UNIQUE
WHERE
    1=1
  AND COMP_CODE IN ('0085',
    '2926')
...
FROM
    OPL OPL
        JOIN
    `bcs-customer360-nonprod.staging_eu.crm_opl_xref` CRM
    ON
            ...
            AND TRIM(opl_source_system_code) = 'SAP-QBC-C'